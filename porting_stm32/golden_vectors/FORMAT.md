...
| 10/13（符号域） | max 绝对误差 | 24（实测 21.6） |
| 14/15（信道估计） | max 绝对误差 | 18 / 27（实测 15.2 / 24.3） |
| 16/17（符号域/FFT 输出） | max 绝对误差 | 24 / 500（实测 21.6 / 396.2） |

**阶段 17 推导**：阶段 16 输入携带 C TX 成形量化（相对差 2.766e-3，实测
21.6/7800）。前向 FFT（无 1/N）按变换增益放大该差；实测输出差 396.2
（相对差仍 2.766e-3，与输入一致）。容差 500 = 实测 396.2 × 1.26 裕量，
同时覆盖不同帧随机序列下的增益变化。
