...
%SIMULATE_CHAPTER5_CCK Modular simulations for Sections 5.1, 5.2, and 5.3.

if nargin < 1
    options = struct();
end
ensure_module_path();
results = scfde.run_chapter5_cck_suite(options, fileparts(mfilename("fullpath")));
end

function ensure_module_path()
papersDir = fileparts(fileparts(mfilename("fullpath")));
moduleDir = fullfile(papersDir, "modules");
if isempty(which("scfde.run_chapter5_cck_suite"))
    addpath(moduleDir);
end
end
