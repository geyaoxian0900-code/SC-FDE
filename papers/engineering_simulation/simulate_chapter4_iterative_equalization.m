...
%SIMULATE_CHAPTER4_ITERATIVE_EQUALIZATION Modular Chapter 4 Turbo suite.
%   The selectable iterative-equalization algorithms are implemented in
%   +scfde/run_chapter4_turbo_suite.m so the SISO decoder or an equalizer
%   branch can be replaced without modifying this engineering entry point.

if nargin < 1
    options = struct();
end
ensure_module_path();
results = scfde.run_chapter4_turbo_suite(options, fileparts(mfilename("fullpath")));
end

function ensure_module_path()
papersDir = fileparts(fileparts(mfilename("fullpath")));
moduleDir = fullfile(papersDir, "modules");
if isempty(which("scfde.run_chapter4_turbo_suite"))
    addpath(moduleDir);
end
end
...
for k=1:numel(bits)
    u=bits(k); coded(2*k-1)=mod(u+state(2),2);
    coded(2*k)=mod(u+state(1)+state(2),2); state=[u state(1)];
end
end
...
for s=0:3
    ...
    for u=0:1
        ...
        out(s+1,u+1,:)=[mod(u+memory(2),2) mod(u+memory(1)+memory(2),2)];
    end
end
end
