...
%   pathDelays  - optional synthetic path delays in symbol periods
%                 (default [0 1 3]); pathGains defaults to
%                 [1, 0.7*exp(1j*0.5), 0.3*exp(-1j*0.8)]
...
link = struct("trainingSymbols", 64, "dataSymbols", dataSymbols, ...
    ...
    "pathDelays", field_default(cfg, "pathDelays", [0, 1, 3]), ...
    "pathGains", field_default(cfg, "pathGains", [1, 0.7 * exp(1j * 0.5), 0.3 * exp(-1j * 0.8)]), ...
    ...
    "dpllProportionalGain", 0.020, "dpllIntegralGain", 0.002, ...
...
% Chip-level noise: snrDb is the post-despread decision SNR.  The
% matched-filter correlation integrates codeLength chips (signal energy
% 1, noise variance nv per chip -> decision SNR = 1/nv), so nv =
% 10^(-snrDb/10) puts the waterfall at -5..3 dB for 4-ary CSK with the
% 63-chip processing gain visible as a shift vs the other scenarios.
nv = 10^(-cfg.snrDb / 10);
