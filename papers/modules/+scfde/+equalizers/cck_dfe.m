...
receiver = scfde.equalizers.pack_equalizer("CCK-DFE", "cck-dfe", ...
    ...
    struct("indices", detected, "scores", scores, ...
    "formulaStatus", "BOOK-EXACT", ...
    "formulaMode", "book", ...
    "bookExperimentEquivalent", false, ...
    "effectiveParameters", struct("candidateLimit", limit), ...
    "formulaNote", "(5-40)~(5-47): forward DFE with temporary decisions from past blocks only (the current word never enters its own feedback); codebook decision + bitTable Hamming BER"));
end
