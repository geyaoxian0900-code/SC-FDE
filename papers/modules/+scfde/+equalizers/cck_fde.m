...
receiver = scfde.equalizers.pack_equalizer("CCK-FDE-IBDFE", "cck-fde", ...
    ...
    "effectiveParameters", struct("iterations", iterations), ...
    "formulaNote", "(5-80) X_hat = C Y - B X_bar with the production MMSE coefficient form (ALG-EQUIV); (5-81)/(5-82) posterior-mean soft feedback with the exponential likelihood; the previous fixed 0.65/0.35 soft mixing and residual-energy rollback are REMOVED (spec 5.6 forbids them)"));
end
