...
receiver = scfde.equalizers.pack_equalizer("CCK-TR-Diversity", ...
    ...
    "branchSoftOutputNote", "branches are the exact BiDFE-2 kernels (5-52)~(5-54) from book/P133.png~P137.png; branch-level certification promotion deferred to Task 9", ...
    "headRegionStatus", "ENGINEERING", ...
    "headRegionNote", "first 'memory' chips have no reversed-branch window: forward branch alone", ...
    "formulaNote", "weakest-link certification: (5-57) equal-weight 1/2 merge after same-time-order restoration is BOOK-EXACT and the branches are the strict BiDFE-2 kernels, but branch-level certification and (5-59) iteration are promoted only in Task 9, so the registered method as a whole is ALG-EQUIV", ...
