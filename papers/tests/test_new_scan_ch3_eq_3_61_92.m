...
tests = functiontests({ ...
    ...
    @testTable32ChannelLockedValues, ...
    @testTable32FractionalDelayPolicy, ...
    @testTable32DefaultQuickChannelsUnchanged, ...
    @testTable32BookModeBankFinite});
...
function testTable32ChannelLockedValues(testCase)
% Table 3-2 (book/P68.png): five taps with locked gains
% [0.5791, 0.6929, 0.3370, 0.1938, 0.1831] and delays
% [0, 9.8, 16.4, 26.0, 31.1] ms.  At 10 kHz every delay is an integer
% number of samples ([0, 98, 164, 260, 311]), so the exact tap grid is
% recoverable without any fractional-delay policy.
sampleRate = 10000;
[channel, info] = scfde.equalizers.ch3_table32_channel(sampleRate);
verifyEqual(testCase, info.delayMilliseconds, [0, 9.8, 16.4, 26.0, 31.1], ...
    "AbsTol", 0, "raw delays must be the locked Table 3-2 values");
verifyEqual(testCase, info.rawGains, [0.5791, 0.6929, 0.3370, 0.1938, 0.1831], ...
    "AbsTol", 0, "raw gains must be the locked Table 3-2 values");
verifyEqual(testCase, info.fractionalMethod, "none", ...
    "at 10 kHz every delay is an integer sample count");
positions = round(info.delaySamples);
verifyEqual(testCase, positions, [0, 98, 164, 260, 311]);
% Non-normalized tap placement: after unit-energy normalization the
% ratios of the tap magnitudes must equal the raw gain ratios.
tapMagnitudes = abs(channel(positions + 1));
expectedRatios = info.rawGains / info.rawGains(1);
verifyEqual(testCase, tapMagnitudes / tapMagnitudes(1), expectedRatios, ...
    "AbsTol", 1e-9, ...
    "the normalized channel must preserve the raw gain ratios");
verifyEqual(testCase, norm(channel), 1, "AbsTol", 1e-12, ...
    "the channel is normalized to unit energy");
end

function testTable32FractionalDelayPolicy(testCase)
% Silent rounding is PROHIBITED: at 4 kHz the delays are NOT integer
% samples, so the default policy must raise
% SCFDE:BookParameterUnavailable instead of rounding; an explicit
% "linear" policy is required and must be recorded.
sampleRate = 4000;    % 9.8 ms -> 39.2 samples (fractional)
verifyError(testCase, @() scfde.equalizers.ch3_table32_channel(sampleRate), ...
    "SCFDE:BookParameterUnavailable", ...
    "non-integer delays must raise instead of being rounded silently");
[channel, info] = scfde.equalizers.ch3_table32_channel(sampleRate, "linear");
verifyEqual(testCase, info.fractionalMethod, "linear", ...
    "the explicit fractional-delay method must be recorded");
verifyTrue(testCase, all(isfinite(channel)), ...
    "the linear-fractional channel must be finite");
verifyEqual(testCase, norm(channel), 1, "AbsTol", 1e-12, ...
    "unit-energy normalization must hold for the fractional channel");
% The linear policy must place the tap energy around the fractional
% positions (no energy at a silently rounded single position).
fracPos = info.delaySamples(2);     % 39.2 -> spread over samples 39/40
verifyTrue(testCase, fracPos ~= round(fracPos), "premise: fractional delay");
verifyTrue(testCase, abs(channel(floor(fracPos) + 1)) > 0 && ...
    abs(channel(floor(fracPos) + 2)) > 0, ...
    "linear interpolation must spread the tap over two adjacent samples");
end

function testTable32DefaultQuickChannelsUnchanged(testCase)
% The default quick channels must be UNCHANGED: without the opt-in
% chapter3ChannelMode the synthetic path table drives the QPSK bank and
% no Table 3-2 routing happens.
res = run_unified_equalizer(struct("equalizers", "all", ...
    "scenario", "qpsk", "frameCount", 1, "makePlot", false, ...
    "randomSeed", 42));
verifyTrue(testCase, isfield(res.config, "pathDelays"), ...
    "the default QPSK link keeps the synthetic path table");
verifyTrue(testCase, ~isfield(res.config, "chapter3Table32"), ...
    "the Table 3-2 metadata must NOT leak into the default path");
verifyTrue(testCase, all(isfinite(res.ber)), ...
    "the default 17-method bank must stay finite");
end

function testTable32BookModeBankFinite(testCase)
% Opt-in reference mode: with channelMode="book-table-3-2" the QPSK bank
% runs on the recovered Table 3-2 channel (linear fractional policy at
% the 4 kHz link rate), the metadata is traced, and the BER stays
% finite.
res = run_unified_equalizer(struct("equalizers", "all", ...
    "scenario", "qpsk", "frameCount", 1, "makePlot", false, ...
    "randomSeed", 42, "channelMode", "book-table-3-2", ...
    "chapter3FractionalDelay", "linear"));
verifyTrue(testCase, all(isfinite(res.ber)), ...
    "the Table 3-2 17-method bank must stay finite");
verifyTrue(testCase, isfield(res.config, "chapter3Table32"), ...
    "the Table 3-2 metadata must be traced into the link");
verifyEqual(testCase, res.config.chapter3Table32.fractionalMethod, "linear", ...
    "the fractional method must be recorded");
verifyEqual(testCase, ...
    res.config.chapter3Table32.delayMilliseconds, ...
    [0, 9.8, 16.4, 26.0, 31.1], "AbsTol", 0, ...
    "the raw millisecond delays must be traced");
end
