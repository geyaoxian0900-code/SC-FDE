...
% BOOK path ignores any reliability weighting: the new (3-61)/(3-62)
% implementation never reads cfg.htfdeReliabilityMode, so all three
% settings must produce identical outputs.  The reliability-weighted
% behavior lives only in htfde_engineering (legacy segmented variant;
% posterior vs none modes must differ in the first iteration).
dataSymbols = 192;
...
ch = struct("received", received, "impulse", imp, ...
    "branches", received, "branchImpulses", imp);
src = struct("data", data.', "tx", tx.', "training", tx(1:32).');
baseCfg = struct("noiseVariance", 0.01, "dataSymbols", dataSymbols, ...
    "uwLength", 64, "fftSize", N, "htfdeMode", "book", ...
    "htfdeSubarrayCount", 1, "htfdeElementsPerSubarray", 1, ...
    "channelEstimateLength", numel(imp), ...
    "feedforwardTaps", 8, "feedbackTaps", 4, "trainingSymbols", 32, ...
    "modulation", "qpsk", "snrDb", 20, ...
...
verifyEqual(testCase, r1.outputs{1}, r2.outputs{1}, ...
    "book path ignores reliability mode (new 3-61/3-62 implementation)");
verifyEqual(testCase, r2.outputs{1}, r3.outputs{1}, ...
    "book path ignores reliability mode (new 3-61/3-62 implementation)");
