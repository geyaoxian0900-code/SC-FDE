...
function testStringArrayExplicitScenario(testCase)
% Same-scenario string arrays must run in an explicit scenario, and
% cross-scenario string arrays must raise SCFDE:MixedScenarios
% (regression: the validator only checked the first element).
result = run_unified_equalizer(struct("equalizers", ...
    ["htfde", "mmse-fde"], "scenario", "qpsk", ...
    "frameCount", 1, "makePlot", false, "randomSeed", 42));
verifyEqual(testCase, numel(result.ids), 2);
verifyError(testCase, ...
    @() run_unified_equalizer(struct("equalizers", ...
        ["htfde", "cck-rake"], "scenario", "qpsk", ...
        "frameCount", 1, "makePlot", false, "randomSeed", 42)), ...
    "SCFDE:MixedScenarios");
end
