...
- **已按审计修正**：
  - 图 3.7-3.10 的估计误差曲线由"参数化噪声模型"改为**真实信号级估计器**
    （过采样 UW 相关联合搜索，式 3-9~3-15 语义）
  - 第 4 章 LDPC 输入噪声方差由经验缩放（0.75/0.85/0.35）改为**均衡后解析残余方差**
    `σ_w²·mean(|W|²)`
  - 子阵 PTR 由"频带正则化逆滤波"改为**纯时反匹配滤波**（式 2-48 语义）
  - 扩展 CCK（16/32 码片）修复调用不存在的 `extend_scfde` 包问题
  - FD-DFE/IBDFE 高 SNR 断言从"必须更优"放宽为"不得显著劣化"（MMSE 已收敛时
    反馈无增益属正常）
