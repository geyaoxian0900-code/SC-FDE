...
- **MATLAB 仿真**：覆盖教材《单载波水声通信技术》第2-6章核心算法的仿真（约 1.9 万行代码），部分逐公式验证、部分趋势复现（详见 `FORMULA_TRACEABILITY.md`）
...
图1章为水声通信系统概述与基础理论（纯理论，本项目不含第1章仿真入口）；第2-6章核心算法均有对应实现，但复现精度不等：**部分逐公式验证、部分趋势复现、部分工程近似**（详见 `FORMULA_TRACEABILITY.md` 公式追踪矩阵与本文"审计说明"节）。
