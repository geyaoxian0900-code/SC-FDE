...
#include "bsp_usart.h"
/**
 * @file bsp_passband_rx.c
 * @brief PA0/ADC0 receiver with signal-triggered preroll and TIMER2 DMA.
 *
 * Detection examines 32-sample peak-to-peak ranges. A 256-sample circular
 * preroll retains frame onset. DMA appends the remaining 48 kHz samples into
 * one contiguous buffer for synchronization and decoding.
 */
...
#define PASSBAND_RX_SAMPLE_RATE_HZ         SCFDE_RX_SAMPLE_RATE_HZ
...
static void passband_rx_copy_preroll_to_buffer(uint32_t preroll_count, uint32_t preroll_write_index)
{
    ...
    /* A full preroll is circular; a partial preroll is linear from zero. */
...
{
    /* TIMER2 TRGO starts each ADC conversion; DMA stores each result. */
...
{
    /* Poll continuous ADC conversion without consuming frame DMA storage. */
...
static uint32_t passband_rx_get_timer_clock_hz(void)
{
    ...
    /* Account for timer clock doubling when APB1 is prescaled. */
    if(apb1_divider == 1u)
    {
...
static void passband_rx_start_dma_internal(uint32_t start_index, uint32_t length)
{
    ...
    /* start_index permits append after copied preroll samples. */
...
{
    /* ISR only acknowledges, stops capture, and exposes completion. */
