...
/**
 * @file bsp_passband_tx.c
 * @brief PA4/DAC0 passband output paced by TIMER6 at 96 kHz.
 *
 * CPU mode generates modem samples on demand and avoids a frame-sized buffer.
 * DMA mode uses a 512-entry staging buffer. Both paths add the 2048-code DAC
 * midpoint, clip to 12 bits, and block until transmission completes.
 */
//==================== 负责用DAC+TIMER6发射通带波形 ====================
...
#define PASSBAND_TX_SAMPLE_RATE_HZ       SCFDE_TX_SAMPLE_RATE_HZ
...
static uint32_t passband_tx_get_timer_clock_hz(void)
{
    ...
    /* Timers use APB clock at divider one and twice APB clock otherwise. */
    if(apb1_divider == 1u)
    {
...
{
    /* On-demand generation avoids storing all 4608 samples in MCU RAM. */
    timer_disable(PASSBAND_TX_TIMER);
    dma_channel_disable(PASSBAND_TX_DMA_PERIPH, PASSBAND_TX_DMA_CHANNEL);
    dma_flag_clear(PASSBAND_TX_DMA_PERIPH, PASSBAND_TX_DMA_CHANNEL, PASSBAND_TX_DMA_ALL_FLAGS);
    dac_dma_disable(PASSBAND_TX_DAC_CHANNEL);
...
{
    /* Each TIMER6 update requests one staged DAC word through DMA. */
    timer_disable(PASSBAND_TX_TIMER);
    dma_channel_disable(PASSBAND_TX_DMA_PERIPH, PASSBAND_TX_DMA_CHANNEL);
    dma_flag_clear(PASSBAND_TX_DMA_PERIPH, PASSBAND_TX_DMA_CHANNEL, PASSBAND_TX_DMA_ALL_FLAGS);
    dac_flag_clear(PASSBAND_TX_DAC_CHANNEL, DAC_FLAG_DDUDR0);
...
void passband_tx_send_blocking(const int16_t *samples, uint32_t length)
{
    ...
    /* NULL samples selects modem callback generation instead of an array. */
    ...
    for(index = 0; index < length; index++)
    {
        ...
        {
            sample_value = scfde_modem_get_tx_sample(index);
        }
